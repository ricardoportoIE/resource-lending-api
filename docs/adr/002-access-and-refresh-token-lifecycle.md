...
- Authorization uses `STUDENT`, `STAFF` and `ADMIN`. Loans and reservations reference the authenticated user directly, so ownership checks do not depend on a separate customer record.
...
Compromised refresh tokens can be revoked and replay becomes detectable. Database-backed rotation adds one transaction per refresh and locks competing refresh attempts so only one can succeed. Logout cannot immediately invalidate an already issued JWT without adding an access-token denylist; the 15-minute lifetime bounds that exposure. The final loan and reservation models reference the identity directly.
